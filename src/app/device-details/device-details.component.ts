...
export class DeviceDetailsComponent implements OnInit {
  ...
  @Input() triggeredRulesName: Array<DataTimeValue> =
    new Array<DataTimeValue>();
  ...
  conflictDetail() {
    ...
    if (this.selectedDevice.conflictReasons.length > 0) {
      const conflictChart = echarts.init(
        document.getElementById('conflictOptionId')!
      );
    ...
    setInterval(() => {
      ...
      if (this.conflictTime != conflictTime) {
        ...
        for (let i = 0; i < this.selectedDevice.conflictReasons.length; i++) {
          var conflictStateTime =
            this.selectedDevice.conflictReasons[i].conflict;
  ...
  frequentChangeDetail() {
    ...
    const frequentChangeChart = echarts.init(
      document.getElementById('changeFrequentId')!
    );
