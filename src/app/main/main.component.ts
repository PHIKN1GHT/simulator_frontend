...
export class MainComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('chart');
    const scenesTreeChart = echarts.init(
      document.getElementById('scenesTreeid')!
    );
    ...
    if (this.scenesTree != null && this.environmentModel != null) {
      ...
      this.dynamicAnalysisService
        .simulateAllScenarios(
          this.environmentModel.devices,
          this.scenesTree,
          this.initModelFileName
        )
        .subscribe((scenes) => {
          this.scenes = scenes;
          if (scenes == null) {
  ...
  generateInteractiveEnvironment() {
    ...
    if (this.modelLayer == null) {
      ...
      this.ruleTextFinal = this.ruleText;
      this.modelGenerateService
        .genererateInteractiveEnvironment(
          this.ruleTextFinal,
          this.modelLayer,
          this.instanceLayer
        )
        .subscribe((instanceAndRules) => {
          ...
          for (
            let i = 0;
            i < this.interactiveInstances.deviceInstances.length;
            i++
          ) {
            var deviceType =
  ...
  staticAnalysis() {
    ...
    this.staticAnalysisService
      .getStaticAnalysis(this.rules, this.interactiveInstances)
      .subscribe((staticAnalysis) => {
  ...
  simulateSingleScenario() {
    if (this.singleModelFileName != '' && this.instanceLayer != null) {
      console.log('...');
      this.modelGenerateService
        .simulateSingleScenario(this.singleModelFileName, this.instanceLayer)
        .subscribe((scenario) => {
