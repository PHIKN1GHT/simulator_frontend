...
import {
  AllRuleAnalysisResult,
  CauseRule,
  CauseRulesCount,
  DeviceAnalysisResult,
  DeviceAnalysisSyntheticResult,
  DeviceCauseRuleConclusion,
  EnvironmentModel,
  PropertyReachableSyntheticResult,
...
export class RuleAnalysisComponent implements OnInit {
  ...
  searchAllScenariosConflict() {
    this.dynamicAnalysisService
      .searchAllScenariosConflict(this.scenarios)
      .subscribe((scenarios) => {
        ...
        for (var i = 0; i < this.scenarios.length; i++) {
          for (var j = 0; j < this.scenarios[i].deviceConflicts.length; j++) {
            if (
              this.scenarios[i].deviceConflicts[j].conflictTimeValues.length > 0
            ) {
              var exist = false;
              for (var k = 0; k < this.deviceAnalysiss.length; k++) {
                ///找到对应的设备分析结果
                if (
                  this.scenarios[i].deviceConflicts[j].instanceName ==
                  this.deviceAnalysiss[k].instanceName
                ) {
                  exist = true;
                  this.deviceAnalysiss[k].hasConflict = true;
                  this.deviceAnalysiss[k].conflictScenarios.push(
                    this.scenarios[i].scenarioName
                  );
                  break;
                }
              }
              if (!exist) {
                ...
                deviceAnalysis.instanceName =
                  this.scenarios[i].deviceConflicts[j].instanceName;
                deviceAnalysis.hasConflict = true;
                deviceAnalysis.conflictScenarios.push(
                  this.scenarios[i].scenarioName
                );
  ...
  locateAllScenariosConflict() {
    this.dynamicAnalysisService
      .locateAllScenariosConflict(
        this.scenarios,
        this.instanceLayer.deviceInstances,
        this.rules,
        this.ifdFileName
      )
      .subscribe((allSynthesizedDeviceAllStatesRuleAndPreRules) => {
        console.log(allSynthesizedDeviceAllStatesRuleAndPreRules);
        this.conflictCausingRules =
          allSynthesizedDeviceAllStatesRuleAndPreRules;
      });
  }
      ...
      .subscribe((scenarios) => {
        ...
        for (var i = 0; i < this.scenarios.length; i++) {
          for (var j = 0; j < this.scenarios[i].deviceJitters.length; j++) {
            if (
              this.scenarios[i].deviceJitters[j].jitterTimeValues.length > 0
            ) {
              var exist = false;
              for (var k = 0; k < this.deviceAnalysiss.length; k++) {
                ///找到对应的设备分析结果
                if (
                  this.scenarios[i].deviceJitters[j].instanceName ==
                  this.deviceAnalysiss[k].instanceName
                ) {
                  exist = true;
                  this.deviceAnalysiss[k].hasJitter = true;
                  this.deviceAnalysiss[k].jitterScenarios.push(
                    this.scenarios[i].scenarioName
                  );
                  break;
                }
              ...
              if (!exist) {
                ...
                deviceAnalysis.jitterScenarios.push(
                  this.scenarios[i].scenarioName
                );
                this.deviceAnalysiss.push(deviceAnalysis);
              }
  ...
  getOtherAnalysis() {
    this.dynamicAnalysisService
      .getOtherAnalysis(this.scenarios, this.interactiveInstances)
      .subscribe((otherAnalysis) => {
        ...
        for (let i = 0; i < otherAnalysis.notTriggeredRulesInAll.length; i++) {
          for (let j = 0; j < this.rules.length; j++) {
            if (
              otherAnalysis.notTriggeredRulesInAll[i] == this.rules[j].ruleName
            ) {
        ...
        for (
          let i = 0;
          i < otherAnalysis.homeBoundedOutBoundedResults.length;
          i++
        ) {}
      });
  }
  ...
  addProperty() {
    ...
    if (this.property.trim() != '') {
      ...
      for (var i = 0; i < this.properties.length; i++) {
        ...
        var existCon;
        second: for (var j = 0; j < conList.length; j++) {
          existCon = false;
  ...
  syntheticAllPropertyReachableReason() {
    ...
    for (let i = 0; i < this.propertyVerifyResults.length; i++) {
      if (this.propertyVerifyResults[i].reachable) {
        ////可达，综合原因
        var reachableCauseRulesCounts = this.syntheticPropertyReachableReason(
          this.propertyVerifyResults[i].reachableReasons
        );
        var propertyReachableSyntheticResult: PropertyReachableSyntheticResult =
          {
            ...
            correspondingRules:
              this.propertyVerifyResults[i].correspondingRules,
          };
        this.propertyReachableSyntheticResults.push(
          propertyReachableSyntheticResult
        );
        console.log(this.propertyReachableSyntheticResults);
      } else {
  ...
  ////////综合property可达的原因
  syntheticPropertyReachableReason(
    reachableReasons: Array<ReachableReason>
  ): Array<CauseRulesCount> {
    ...
    for (let i = 0; i < reachableReasons.length; i++) {
      ...
      for (let j = 0; j < reachableCauseRulesCounts.length; j++) {
        if (
          this.isStatesSame(
            reachableReason.causingRules,
            reachableCauseRulesCounts[j].causingRules
          )
        ) {
          exist = true;
          ...
          if (reachableCauseRulesCounts[j].exsitScenes.length == 0) {
            ...
            reachableCauseRulesCounts[j].exsitScenes[
              reachableCauseRulesCounts[j].exsitScenes.length - 1
            ] != scenarioName
          ) {
            reachableCauseRulesCounts[j].exsitScenes.push(scenarioName);
          }
  ...
  conclusion() {
    ...
    for (let i = 0; i < this.deviceAnalysisSyntheticResults.length; i++) {
      if (
        this.deviceAnalysisSyntheticResults[i].conflictCauseRulesCounts.length >
        0
      ) {
        ////conflcit
        ...
        for (
          let j = 0;
          j <
          this.deviceAnalysisSyntheticResults[i].conflictCauseRulesCounts
            .length;
          j++
        ) {
          var conflictCauseRulesCount =
            this.deviceAnalysisSyntheticResults[i].conflictCauseRulesCounts[j];
          for (
            let k = 0;
            k < conflictCauseRulesCount.causingRules.length;
            k++
          ) {
            ...
            for (
              let m = 0;
              m < deviceConflictConflusion.causingRules.length;
              m++
            ) {
              if (
                deviceConflictConflusion.causingRules[m].state ===
                causingRule.state
              ) {
                existState = true;
                ///再看规则是否存在
                for (let n = 0; n < causingRule.stateCausingRules.length; n++) {
                  ...
                  for (
                    let l = 0;
                    l <
                    deviceConflictConflusion.causingRules[m].stateCausingRules
                      .length;
                    l++
                  ) {
                    if (
                      causingRule.stateCausingRules[n].rule.ruleName ===
                      deviceConflictConflusion.causingRules[m]
                        .stateCausingRules[l].rule.ruleName
                    ) {
                      exsitRule = true;
                      break;
                    }
                  ...
                  if (!exsitRule) {
                    deviceConflictConflusion.causingRules[
                      m
                    ].stateCausingRules.push(causingRule.stateCausingRules[n]);
                  }
                }
                break;
              }
      ...
      if (
        this.deviceAnalysisSyntheticResults[i].jitterCauseRulesCounts.length > 0
      ) {
        ////Jitter
        ...
        for (
          let j = 0;
          j <
          this.deviceAnalysisSyntheticResults[i].jitterCauseRulesCounts.length;
          j++
        ) {
          var jitterCauseRulesCount =
            this.deviceAnalysisSyntheticResults[i].jitterCauseRulesCounts[j];
          for (let k = 0; k < jitterCauseRulesCount.causingRules.length; k++) {
            ...
            for (
              let m = 0;
              m < deviceJitterConflusion.causingRules.length;
              m++
            ) {
              if (
                deviceJitterConflusion.causingRules[m].state ===
                causingRule.state
              ) {
                existState = true;
                ///再看规则是否存在
                for (let n = 0; n < causingRule.stateCausingRules.length; n++) {
                  ...
                  for (
                    let l = 0;
                    l <
                    deviceJitterConflusion.causingRules[m].stateCausingRules
                      .length;
                    l++
                  ) {
                    if (
                  ...
                  if (!exsitRule) {
                    deviceJitterConflusion.causingRules[
                      m
                    ].stateCausingRules.push(causingRule.stateCausingRules[n]);
                  }
                }
  ...
  syntheticDeviceReason() {
    if (this.environmentModel != null && this.scenes != null) {
      ...
      for (let i = 0; i < devices.length; i++) {
        ...
        for (let j = 0; j < this.scenes.length; j++) {
          ...
          if (this.scenes[j].deviceAnalysisResults != null) {
            for (
              let k = 0;
              k < this.scenes[j].deviceAnalysisResults.length;
              k++
            ) {
              if (
                devices[i].deviceName ===
                this.scenes[j].deviceAnalysisResults[k].deviceName
              ) {
                /////找到对应设备，并合并
                this.syntheticConflictReason(
                  this.scenes[j].deviceAnalysisResults[k],
                  conflictCauseRulesCounts,
                  this.scenes[j].scenarioName
                );
                this.syntheticJitterReason(
                  this.scenes[j].deviceAnalysisResults[k],
                  jitterCauseRulesCounts,
                  this.scenes[j].scenarioName
                );
                break;
              }
        ...
        if (
          jitterCauseRulesCounts.length > 0 ||
          conflictCauseRulesCounts.length > 0
        ) {
      ...
      for (let i = 0; i < this.deviceAnalysisSyntheticResults.length; i++) {
        if (
          deviceAnalysisSyntheticResults[i].conflictCauseRulesCounts.length > 0
        ) {
          this.hasConflict = true;
        }
        if (
          deviceAnalysisSyntheticResults[i].jitterCauseRulesCounts.length > 0
        ) {
          this.hasJitter = true;
        }
        ...
        for (let j = 0; j < jitterCauseRulesCounts.length; j++) {
          if (
            ...
            if (jitterCauseRulesCounts[j].exsitScenes.length == 0) {
              ...
              jitterCauseRulesCounts[j].exsitScenes[
                jitterCauseRulesCounts[j].exsitScenes.length - 1
              ] != scenarioName
            ) {
              jitterCauseRulesCounts[j].exsitScenes.push(scenarioName);
            }
        ...
        for (let j = 0; j < conflictCauseRulesCounts.length; j++) {
          if (
            ...
            if (conflictCauseRulesCounts[j].exsitScenes.length == 0) {
              ...
              conflictCauseRulesCounts[j].exsitScenes[
                conflictCauseRulesCounts[j].exsitScenes.length - 1
              ] != scenarioName
            ) {
              conflictCauseRulesCounts[j].exsitScenes.push(scenarioName);
            }
