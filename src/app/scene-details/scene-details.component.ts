...
export class SceneDetailsComponent implements OnInit {
  ...
  devicesConflictsAllCausingRules = new Array<
    Array<Array<DeviceStateAndCausingRules>>
  >(); ////各冲突设备每次发生冲突的原因
  devicesConflictsSynthesizedCausingRules = new Array<
    Array<Array<DeviceStateAndCausingRules>>
  >(); ////各冲突设备冲突发生的综合原因
  selectedDeviceConflictsAllCausingRules = new Array<
    Array<DeviceStateAndCausingRules>
  >(); ///选中设备每次发生冲突的原因
  selectedDeviceConflictsSynthesizedCausingRules = new Array<
    Array<DeviceStateAndCausingRules>
  >(); ////选中设备冲突发生的综合原因

  selectedScenarioDevicesJitters = new Array<DeviceJitter>();
  devicesJittersAllCausingRules = new Array<
    Array<Array<DeviceStateAndCausingRules>>
  >();
  devicesJittersSynthesizedCausingRules = new Array<
    Array<Array<DeviceStateAndCausingRules>>
  >();
  selectedDeviceJittersAllCausingRules = new Array<
    Array<DeviceStateAndCausingRules>
  >();
  selectedDeviceJittersSynthesizedCausingRules = new Array<
    Array<DeviceStateAndCausingRules>
  >();
  ...
  getJitterOption(
    selectedDeviceJitter: DeviceJitter,
    ...
    for (var i = 0; i < this.interactiveInstances.deviceInstances.length; i++) {
      /////找到对应设备
      if (
        dataTimeValue.instanceName ===
        this.interactiveInstances.deviceInstances[i].instanceName
      ) {
        var deviceType =
          this.interactiveInstances.deviceInstances[i].deviceType;
    ...
    for (let i = 0; i < selectedDeviceJitter.jitterTimeValues.length; i++) {
      ...
      for (
        var j = 0;
        j < selectedDeviceJitter.jitterTimeValues[i].length;
        j++
      ) {
    ...
    this.jitterOption = {
      ...
      tooltip: {
        ...
        formatter: function (params: any) {
          ...
          var time: string =
            params[0].marker + '<b>time</b>: ' + params[0].axisValueLabel;
  ...
  getConflictOption(selectedDeviceConflict: DeviceConflict) {
    ...
    if (selectedDeviceConflict.conflictTimeValues.length > 0) {
      for (
        let i = 0;
        i < selectedDeviceConflict.conflictTimeValues.length;
        i++
      ) {
  ...
  syntheticJitterReason() {
    if (this.selectedDeviceAnalysisResult != null) {
      ...
      for (
        let i = 0;
        i < this.selectedDeviceAnalysisResult.jitterReasons.length;
        i++
      ) {
        ////看是否已经存在
        ...
        for (let j = 0; j < jitterCauseRulesCounts.length; j++) {
          if (
            this.isStatesSame(
              jitterReason.causingRules,
              jitterCauseRulesCounts[j].causingRules
            )
          ) {
            exist = true;
  ...
  syntheticConflictReason() {
    if (this.selectedDeviceAnalysisResult != null) {
      ...
      for (
        let i = 0;
        i < this.selectedDeviceAnalysisResult.conflictReasons.length;
        i++
      ) {
        ////看是否已经存在
        var conflictReason =
          this.selectedDeviceAnalysisResult.conflictReasons[i];
        ...
        for (let j = 0; j < conflictCauseRulesCounts.length; j++) {
          if (
            this.isStatesSame(
              conflictReason.causingRules,
              conflictCauseRulesCounts[j].causingRules
            )
          ) {
            exist = true;
  ...
  isStateCausingRuleSame(
    stateCausingRules1: Array<RuleNode>,
    stateCausingRules2: Array<RuleNode>
  ): boolean {
    ...
    for (let i = 0; i < stateCausingRules1.length; i++) {
      ...
      for (let j = 0; j < stateCausingRules2.length; j++) {
        if (
          stateCausingRules2[j].rule.ruleName ===
          stateCausingRules1[i].rule.ruleName
        ) {
          exist = true;
          break;
        }
  ...
  /////两则状态及规则是否一样
  isStatesSame(
    causingRules1: Array<CauseRule>,
    causingRules2: Array<CauseRule>
  ): boolean {
    ...
    for (let i = 0; i < causingRules1.length; i++) {
      ...
      for (let j = 0; j < causingRules2.length; j++) {
        if (causingRules2[j].state === causingRules1[i].state) {
          ...
          ////看规则是否一样
          if (
            !this.isStateCausingRuleSame(
              causingRules2[j].stateCausingRules,
              causingRules1[i].stateCausingRules
            )
          ) {
            ////两组规则不一样
            return false;
  ...
  calculateComfortLevel() {
    ...
    if (parseFloat(this.highValue) < parseFloat(this.lowValue)) {
      ...
      this.singleScenarioAnlaysisService
        .getAttributeSatisfaction(
          this.selectAttributeName,
          this.lowValue,
          this.highValue,
          this.selectedScenario.dataTimeValues
        )
        .subscribe((satis) => {
          console.log(satis);
  ...
  energyConsumption() {
    ...
    this.singleScenarioAnlaysisService
      .getEnergyConsumption(
        this.selectedScenario.dataTimeValues,
        this.interactiveInstances.deviceInstances
      )
      .subscribe((deviceConsumptions) => {
  ...
  getTotalConsumption(deviceConsumptions: Array<Array<string>>) {
    ...
    for (let i = 0; i < deviceConsumptions.length; i++) {
      var consumption =
        (parseFloat(deviceConsumptions[i][1]) *
          parseFloat(this.equivalentTime)) /
        parseFloat(this.simulationTime) /
        1000; ///单位度  kw*h
  ...
  showConflictAnalysis() {
    ...
    this.selectedDeviceConflictsAllCausingRules = new Array<
      Array<DeviceStateAndCausingRules>
    >(); ///选中设备每次发生冲突的原因
    this.selectedDeviceConflictsSynthesizedCausingRules = new Array<
      Array<DeviceStateAndCausingRules>
    >(); ////选中设备冲突发生的综合原因
    for (let i = 0; i < this.selectedScenario.deviceConflicts.length; i++) {
      if (
        this.selectedScenarioDevicesConflicts[i].instanceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        if (
          this.selectedScenarioDevicesConflicts[i].conflictTimeValues.length > 0
        ) {
    ...
    for (var i = 0; i < this.devicesConflictsAllCausingRules.length; i++) {
      if (
        this.devicesConflictsAllCausingRules[i][0][0].deviceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        this.selectedDeviceConflictsAllCausingRules =
          this.devicesConflictsAllCausingRules[i];
        break;
      }
    }
    for (
      var i = 0;
      i < this.devicesConflictsSynthesizedCausingRules.length;
      i++
    ) {
      if (
        this.devicesConflictsSynthesizedCausingRules[i][0][0].deviceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        this.selectedDeviceConflictsSynthesizedCausingRules =
          this.devicesConflictsSynthesizedCausingRules[i];
        break;
      }
    }
  }
  showJitterAnalysis() {
    ...
    this.selectedDeviceJittersAllCausingRules = new Array<
      Array<DeviceStateAndCausingRules>
    >();
    this.selectedDeviceJittersSynthesizedCausingRules = new Array<
      Array<DeviceStateAndCausingRules>
    >();
    for (let i = 0; i < this.selectedScenario.dataTimeValues.length; i++) {
      if (
        this.selectedScenario.dataTimeValues[i].instanceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        var dataTimeValue = this.selectedScenario.dataTimeValues[i];
        ...
        for (let j = 0; j < this.selectedScenarioDevicesJitters.length; j++) {
          if (
            this.selectedScenarioDevicesJitters[j].instanceName ==
            this.selectedDeviceInstance.instanceName
          ) {
    ...
    for (var i = 0; i < this.devicesJittersAllCausingRules.length; i++) {
      if (
        this.devicesJittersAllCausingRules[i][0][0].deviceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        this.selectedDeviceJittersAllCausingRules =
          this.devicesJittersAllCausingRules[i];
        break;
      }
    }
    for (
      var i = 0;
      i < this.devicesJittersSynthesizedCausingRules.length;
      i++
    ) {
      if (
        this.devicesJittersSynthesizedCausingRules[i][0][0].deviceName ==
        this.selectedDeviceInstance.instanceName
      ) {
        this.selectedDeviceJittersSynthesizedCausingRules =
          this.devicesJittersSynthesizedCausingRules[i];
        break;
      }
    }
  }
  showConsumptionAnalysis() {
    ...
    for (var i = 0; i < this.selectedScenario.dataTimeValues.length; i++) {
      if (
        ...
        this.singleScenarioAnlaysisService
          .calculateDeviceStatesDuration(
            this.selectedScenario.dataTimeValues[i],
            this.selectedDeviceInstance
          )
          .subscribe((deviceStatesDuration) => {
  ...
  getEquivalentTime(timeStr: string): string {
    var time = parseFloat(timeStr);
    var equivalentTime =
      (time / parseFloat(this.simulationTime)) *
      parseFloat(this.equivalentTime);
  ...
  calculateConsumption(): number {
    ...
    for (var i = 0; i < this.selectedDeviceStatesDuration.length; i++) {
      sum +=
        parseFloat(this.selectedDeviceStatesDuration[i][3]) *
        parseFloat(
          this.getEquivalentTime(this.selectedDeviceStatesDuration[i][2])
        );
    }
    return sum;
  }
  ...
  showDevicesAnalysis() {
    this.singleScenarioAnlaysisService
      .searchSingleScenarioConflict(this.selectedScenario)
      .subscribe((scenario) => {
        console.log(scenario);
        this.selectedScenario = scenario;
        this.selectedScenarioDevicesConflicts =
          this.selectedScenario.deviceConflicts;
        this.singleScenarioAnlaysisService
          .locateSingleScenariosAllConflict(
            this.selectedScenario,
            this.rules,
            this.interactiveInstances.deviceInstances,
            this.ifdFileName
          )
          .subscribe((devicesAllStatesRuleAndPreRules) => {
            console.log(devicesAllStatesRuleAndPreRules);
            this.devicesConflictsAllCausingRules =
              devicesAllStatesRuleAndPreRules;
            this.singleScenarioAnlaysisService
              .getCausingRulesSynthesized(devicesAllStatesRuleAndPreRules)
              .subscribe((synthesizedDevicesStatesRuleAndPreRules) => {
                console.log(synthesizedDevicesStatesRuleAndPreRules);
                this.devicesConflictsSynthesizedCausingRules =
                  synthesizedDevicesStatesRuleAndPreRules;
              });
          });
      });
    this.singleScenarioAnlaysisService
      .searchSingleScenarioJitter(
        this.selectedScenario,
        this.intervalTime,
        this.simulationTime,
        this.equivalentTime
      ...
      .subscribe((scenario) => {
        console.log(scenario);
        this.selectedScenario = scenario;
        this.selectedScenarioDevicesJitters =
          this.selectedScenario.deviceJitters;
        this.singleScenarioAnlaysisService
          .locateSingleScenariosAllJitter(
            this.selectedScenario,
            this.rules,
            this.interactiveInstances.deviceInstances,
            this.ifdFileName
          ...
          .subscribe((devicesAllStatesRuleAndPreRules) => {
            console.log(devicesAllStatesRuleAndPreRules);
            this.devicesJittersAllCausingRules =
              devicesAllStatesRuleAndPreRules;
            this.singleScenarioAnlaysisService
              .getCausingRulesSynthesized(devicesAllStatesRuleAndPreRules)
              .subscribe((synthesizedDevicesStatesRuleAndPreRules) => {
                console.log(synthesizedDevicesStatesRuleAndPreRules);
                this.devicesJittersSynthesizedCausingRules =
                  synthesizedDevicesStatesRuleAndPreRules;
              });
          });
      });
